...
# ==============================
learning_guides = {
    "기초역량": "컴퓨터 기본 조작, 파일 관리, 온라인 학습 환경 활용에 대한 기초 보완이 필요합니다.",
    "전공이해": "전공 핵심 개념과 용어 위주의 단계적 학습을 권장합니다.",
...
st.caption("※ 휴대폰 뒤 4자리는 식별 목적으로만 사용됩니다.")
st.divider()
...
if st.button("📊 제출하기"):
    ...
    if submitted_mask.any() and not retry_allowed:
        st.error("이미 제출한 평가입니다. 재응시는 교사 승인 후 가능합니다.")
        st.stop()

    # 재응시 허용 시 기존 결과 삭제
    if submitted_mask.any() and retry_allowed:
        df_all = df_all[~submitted_mask]
        retry_df = retry_df[~(
            (retry_df["이름"] == name) &
            (retry_df["기수"] == class_no) &
            (retry_df["식별자"] == uid)
    ...
    level = (
        "집중관리 필요" if total_rate < 50 else
        "일반 수준" if total_rate < 80 else
        "우수 수준"
    )
...
if password == TEACHER_PASSWORD:
    ...
    if not df_all.empty:
        ...
        if st.button("🔓 재응시 허용"):
            ...
            retry_df.to_csv(RETRY_FILE, index=False)
            st.success(f"{n} 훈련생의 재응시가 허용되었습니다.")
